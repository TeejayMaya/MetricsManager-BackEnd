...
class AdminController extends \Phalcon\Mvc\Controller {
    ...
    public function loginAction()
    {
        ...
        if ($existResult >= 1) {
            ...
            if ($this->security->checkHash($data['password'], $userPasswordHash)) {
                ...
                $logintoken=bin2hex(random_bytes('10')); //Auth login token
    ...
    public function forgotpasswordAction()
    {
        ...
        if ($data['resetcode'] && is_string($data['resetcode'])) {
            ...
            if ($existResult >= 1) {
                $passwordresetcode=bin2hex(random_bytes('8')); //Reset code
    ...
    public function dashboarddataAction()
    {
        ...
        //Passed the User Auth Check, Proceed with the Business Logic
        $totalusers=$users->sumUsers(); // Get User Total users Sum data from the Database
        $totalmerchants=$merchants->sumMerchants(); // Get User Total merchants Sum data from the Database
        $totalproducts=$products->sumProducts(); // Get User Total products Sum data from the Database
        $totalorders=$transactions->sumOrders(); // Get User Total orders Sum data from the Database
        $totalsales=$transactions->sumSales(); // Get User Total sales Sum data from the Database
        $totaladmins=$admin->sumAdmins(); // Get User Total admins Sum data from the Database
        $totalposts=$posts->sumPosts(); // Get User Total Posts Sum data from the Database
        //$totaltickets = $supporttickets->adminTicketsCount(); //Total Support Tickets
        ...
        //Merchant SignUps
        $currentmonth_MerchantsSignupCOUNT=$merchants->merchantsSignupCOUNT($currentmonth); // Get data for graph chart
        $lastmonthone_MerchantsSignupCOUNT=$merchants->merchantsSignupCOUNT($lastmonthone); // Get data for graph chart
        $lastmonthtwo_MerchantsSignupCOUNT=$merchants->merchantsSignupCOUNT($lastmonthtwo); // Get data for graph chart
        $lastmonththree_MerchantsSignupCOUNT=$merchants->merchantsSignupCOUNT($lastmonththree); // Get data for graph chart
        $lastmonthfour_MerchantsSignupCOUNT=$merchants->merchantsSignupCOUNT($lastmonthfour); // Get data for graph chart
        $lastmonthfive_MerchantsSignupCOUNT=$merchants->merchantsSignupCOUNT($lastmonthfive); // Get data for graph chart
        $merchantsSignup_GraphData = array
        (
        array("$currentmonthLABEL","$currentmonth_MerchantsSignupCOUNT"),
        array("$lastmonthoneLABEL","$lastmonthone_MerchantsSignupCOUNT"),
        array("$lastmonthtwoLABEL","$lastmonthtwo_MerchantsSignupCOUNT"),
        array("$lastmonththreeLABEL","$lastmonththree_MerchantsSignupCOUNT"),
        array("$lastmonthfourLABEL","$lastmonthfour_MerchantsSignupCOUNT"),
        array("$lastmonthfiveLABEL","$lastmonthfive_MerchantsSignupCOUNT")
        );
        //Product SignUps
        $currentmonth_ProductsSignupCOUNT=$products->productsSignupCOUNT($currentmonth); // Get data for graph chart
        $lastmonthone_ProductsSignupCOUNT=$products->productsSignupCOUNT($lastmonthone); // Get data for graph chart
        $lastmonthtwo_ProductsSignupCOUNT=$products->productsSignupCOUNT($lastmonthtwo); // Get data for graph chart
        $lastmonththree_ProductsSignupCOUNT=$products->productsSignupCOUNT($lastmonththree); // Get data for graph chart
        $lastmonthfour_ProductsSignupCOUNT=$products->productsSignupCOUNT($lastmonthfour); // Get data for graph chart
        $lastmonthfive_ProductsSignupCOUNT=$products->productsSignupCOUNT($lastmonthfive); // Get data for graph chart
        $productsSignup_GraphData = array
        (
        ...
        //Posts Counts
        $currentmonth_PostsCOUNT=$posts->postsCOUNT($currentmonth); // Get data for graph chart
        $lastmonthone_PostsCOUNT=$posts->postsCOUNT($lastmonthone); // Get data for graph chart
        $lastmonthtwo_PostsCOUNT=$posts->postsCOUNT($lastmonthtwo); // Get data for graph chart
        $lastmonththree_PostsCOUNT=$posts->postsCOUNT($lastmonththree); // Get data for graph chart
        $lastmonthfour_PostsCOUNT=$posts->postsCOUNT($lastmonthfour); // Get data for graph chart
        $lastmonthfive_PostsCOUNT=$posts->postsCOUNT($lastmonthfive); // Get data for graph chart
        $postsCounts_GraphData = array
        (
        ...
        //Jobs Counts
        $currentmonth_JobsCOUNT=$jobs->jobsCOUNT($currentmonth); // Get data for graph chart
        $lastmonthone_JobsCOUNT=$jobs->jobsCOUNT($lastmonthone); // Get data for graph chart
        $lastmonthtwo_JobsCOUNT=$jobs->jobsCOUNT($lastmonthtwo); // Get data for graph chart
        $lastmonththree_JobsCOUNT=$jobs->jobsCOUNT($lastmonththree); // Get data for graph chart
        $lastmonthfour_JobsCOUNT=$jobs->jobsCOUNT($lastmonthfour); // Get data for graph chart
        $lastmonthfive_JobsCOUNT=$jobs->jobsCOUNT($lastmonthfive); // Get data for graph chart
        $jobsCounts_GraphData = array
        (
        array("$currentmonthLABEL","$currentmonth_JobsCOUNT"),
        array("$lastmonthoneLABEL","$lastmonthone_JobsCOUNT"),
        array("$lastmonthtwoLABEL","$lastmonthtwo_JobsCOUNT"),
        array("$lastmonththreeLABEL","$lastmonththree_JobsCOUNT"),
        array("$lastmonthfourLABEL","$lastmonthfour_JobsCOUNT"),
        array("$lastmonthfiveLABEL","$lastmonthfive_JobsCOUNT")
        );
        //Transaction Counts
        ...
        $delivered_TransactionsCOUNT=$transactions->transactionsDeliveryStatisticsCount("delivered"); // Get data for pie-chart graph
        $pending_TransactionsCOUNT=$transactions->transactionsDeliveryStatisticsCount("pending"); // Get data for pie-chart graph
        $intransit_TransactionsCOUNT=$transactions->transactionsDeliveryStatisticsCount("intransit"); // Get data for pie-chart graph
        ...
        $transactionsDeliveryStatisticsData = array
        (
        array("Delivery","$delivered_TransactionsCOUNT"),
        array("Pending","$pending_TransactionsCOUNT"),
        array("Intransit","$intransit_TransactionsCOUNT")
        );

        echo json_encode(array("status"=>"success","totalposts"=>"$totalposts","totalusers"=>"$totalusers","totalmerchants"=>"$totalmerchants","totalproducts"=>"$totalproducts","totalorders"=>"$totalorders","totalsales"=>"$totalsales","totaladmins"=>"$totaladmins","totaltickets"=>"$totaltickets","usersgraphdata"=>$usersSignup_GraphData,"merchantgraphdata"=>$merchantsSignup_GraphData,"productsgraphdata"=>$productsSignup_GraphData,"postsgraphdata"=>$postsCounts_GraphData,"jobsgraphdata"=>$jobsCounts_GraphData,"ordersgraphdata"=>$transactionsCounts_GraphData,"paymentstatisticsdata"=>$transactionsPaymentsStatisticsData,"deliverystatisticsdata"=>$transactionsDeliveryStatisticsData));
    }
    ...
    public function sendnewsletterAction()
    {
        ...
        if($data['type']=="notifications"){
            $response=$generalService->send_pushnotificationtoall($data['title'],$data['message']);
