...
class PageController extends \Phalcon\Mvc\Controller {
    ...
    public function dashboardAction()
    {
        ...
        //Passed the User Auth Check, Proceed with the Business Logic
        //cURL
        $url = "https://api.openweathermap.org/data/2.5/weather?q=lagos,NG&units=metric&appid=e5ec148a1acffe99a22dd06222583b0e";
        $ch = curl_init();
        ...
        $newsheadlines = "";
        $advertorialsData=$advertorials->singleEntryByPosition("mobilecoverad"); // Get Advert data from the Database
        $appcoverbg=$this->getDi()->getShared('siteurl').'/'.$advertorialsData->image;

        //Featured News Healines
        $itemLists = array();
        $itemList = array();
        $resultNum = 5;
        $resultData=$blog->getBlogFeaturedPosts($resultNum); // Get Blog posts data from the Database
        foreach($resultData as $blogData) {
            $newsheadlines .= $blogData->title."... ";
        } 
        echo json_encode(array("status"=>"success","appcoverbg"=>"$appcoverbg","featurednewsheadlines"=>"$newsheadlines","weatherDesc"=>"$desc","weatherTemp"=>"$temp","weatherTempMin"=>"$mintemp","weatherTempMax"=>"$maxtemp","weatherWind"=>"$wind","weatherHumidity"=>"$humidity","weatherPressure"=>"$pressure"));
    }
}
?>